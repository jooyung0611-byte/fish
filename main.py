...
def render_3d_ocean_view(
    ...
    <head>
        <style>
            body {{ margin: 0; overflow: hidden; background: #050B14; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }}
            #canvas-container {{ width: 100%; height: 460px; border-radius: 16px; overflow: hidden; position: relative; box-shadow: 0 10px 30px rgba(0,0,0,0.8); }}
            #ui-overlay {{ position: absolute; top: 12px; left: 12px; color: #00F0FF; font-size: 13px; font-weight: 700; background: rgba(5, 11, 20, 0.75); backdrop-filter: blur(8px); padding: 8px 16px; border-radius: 8px; border: 1px solid rgba(0,240,255,0.4); text-shadow: 0 0 8px rgba(0,240,255,0.6); }}
            #rod-info {{ position: absolute; bottom: 12px; right: 12px; color: #FFFFFF; font-size: 13px; font-weight: 600; background: rgba(5, 11, 20, 0.8); backdrop-filter: blur(8px); padding: 8px 16px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.2); }}
            #status-banner {{ position: absolute; top: 40%; left: 50%; transform: translate(-50%, -50%); color: #FFD700; font-size: 22px; font-weight: 800; text-shadow: 0 0 15px #FF8C00, 0 0 5px #000; display: none; pointer-events: none; text-align: center; width: 90%; background: rgba(0,0,0,0.4); padding: 10px 0; border-radius: 30px; }}
        </style>
    ...
    <body>
        ...
        <script>
            ...
            const camera = new THREE.PerspectiveCamera(55, container.clientWidth / 460, 0.1, 1000);
            ...
            const pointLight = new THREE.PointLight({rod_color_hex}, 2.0, 15);
            pointLight.position.set(2.2, 2.5, 5.0);
            scene.add(pointLight);

            // 고품질 바다 수면
            const oceanGeo = new THREE.PlaneGeometry(80, 80, 80, 80);
            ...
            // 극세사 손잡이 및 디테일 링
            const handleGeo = new THREE.CylinderGeometry(0.1, 0.13, 1.4, 32);
            ...
            const gripRingGeo = new THREE.TorusGeometry(0.12, 0.02, 16, 32);
            const gripRingMat = new THREE.MeshStandardMaterial({{ color: rodColor, metalness: 0.9, roughness: 0.1 }});
            const gripRing = new THREE.Mesh(gripRingGeo, gripRingMat);
            gripRing.position.set(0, -0.2, 0);
            gripRing.rotation.x = Math.PI / 2;
            rodGroup.add(gripRing);

            // 카본 테이퍼드 메인 낚시대
            const mainRodGeo = new THREE.CylinderGeometry(0.02, 0.08, 5.5, 32);
            ...
            mainRodMesh.position.set(0, 2.75, 0);
            rodGroup.add(mainRodMesh);

            // 가이드 링 추가 (디테일 업그레이드)
            for(let g = 1; g <= 5; g++) {{
                const guideGeo = new THREE.TorusGeometry(0.08 - g*0.01, 0.012, 12, 24);
                const guideMat = new THREE.MeshStandardMaterial({{ color: 0xdddddd, metalness: 1.0 }});
                const guide = new THREE.Mesh(guideGeo, guideMat);
                guide.position.set(0, g * 0.9, 0.05);
                rodGroup.add(guide);
            }}
            ...
            if (shapeType === "bamboo") {{
                for(let b=0; b<6; b++) {{
                    const nodeGeo = new THREE.TorusGeometry(0.075 - b*0.007, 0.025, 12, 24);
                    const nodeMat = new THREE.MeshStandardMaterial({{ color: 0x5c3a21, roughness: 0.8 }});
                    const node = new THREE.Mesh(nodeGeo, nodeMat);
                    node.position.set(0, b * 0.85, 0);
                    node.rotation.x = Math.PI / 2;
                    rodGroup.add(node);
                }}
            }} else if (shapeType === "trident" || shapeType === "dragon_horns") {{
                const hornGeo = new THREE.ConeGeometry(0.08, 1.2, 16);
                const hornMat = new THREE.MeshStandardMaterial({{ color: rodColor, metalness: 0.9, roughness: 0.1 }});
                const h1 = new THREE.Mesh(hornGeo, hornMat); h1.position.set(-0.2, 5.2, 0); h1.rotation.z = -0.35;
                const h2 = new THREE.Mesh(hornGeo, hornMat); h2.position.set(0.2, 5.2, 0); h2.rotation.z = 0.35;
                rodGroup.add(h1); rodGroup.add(h2);
            }} else if (shapeType === "wings" || shapeType === "feather") {{
                const wingGeo = new THREE.BoxGeometry(0.02, 1.4, 0.5);
                const wingMat = new THREE.MeshStandardMaterial({{ color: 0xffffff, transparent: true, opacity: 0.85, emissive: 0xffffff, emissiveIntensity: 0.3 }});
                const w1 = new THREE.Mesh(wingGeo, wingMat); w1.position.set(-0.25, 3.0, 0); w1.rotation.z = 0.45;
                const w2 = new THREE.Mesh(wingGeo, wingMat); w2.position.set(0.25, 3.0, 0); w2.rotation.z = -0.45;
                rodGroup.add(w1); rodGroup.add(w2);
            }} else if (shapeType === "neon_rings" || shapeType === "portal_orb") {{
                const ringGeo = new THREE.TorusGeometry(0.35, 0.035, 16, 32);
                const ringMat = new THREE.MeshStandardMaterial({{ color: rodColor, emissive: rodColor, emissiveIntensity: 0.8 }});
            ...
            for(let i=0; i<pCount*3; i++) {{ pPos[i] = (Math.random() - 0.5) * 2.5; }}
            pGeo.setAttribute('position', new THREE.BufferAttribute(pPos, 3));
            const pMat = new THREE.PointsMaterial({{ size: 0.09, color: rodColor, transparent: true, opacity: 0.85, blending: THREE.AdditiveBlending }});
            const particles = new THREE.Points(pGeo, pMat);
            particles.position.set(2.4, 2.5, 5.8);
            scene.add(particles);
            ...
            function animate() {{
                ...
                for (let i = 0; i < pos.count; i++) {{
                    let u = pos.getX(i);
                    let v = pos.getY(i);
                    pos.setZ(i, Math.sin(u * 0.5 + time * 2.5) * 0.2 + Math.cos(v * 0.5 + time * 2.0) * 0.2);
                }}
                ...
                const rodTipWorldPos = new THREE.Vector3(0, 5.5, 0);
